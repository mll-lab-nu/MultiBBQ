...
| Path | Contents |
|---|---|
| `gpt_image_gen/multibbq_visual_language.{csv,json}` | GPT-Image-1 images, visual-language eval |
| `gpt_image_gen/multibbq_visual_only.{csv,json}` | GPT-Image-1 images, visual-only eval |
| `imagen4ultra_image_gen/multibbq_visual_language.{csv,json}` | Imagen-4-Ultra images, visual-language eval |
| `imagen4ultra_image_gen/multibbq_visual_only.{csv,json}` | Imagen-4-Ultra images, visual-only eval |
...
harness expects under `./data/images/`.

**Construction inputs** (used by the notebooks, not by evaluation):

| File | Role |
|---|---|
| `multibbq_template_table.csv` | Revised template table consumed by `notebooks/gen_template.ipynb`. |
| `multibbq_template_table_w_face_id.csv` | Same, with per-image face ids for real-world matching. |
